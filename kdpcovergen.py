...
def draw_credit(canvas):
    canvas.setFillColorRGB(0, 0, 0, 1)
    canvas.setFont(FONT, FONT_SIZE * 0.8)
    text_x = (bleed + (PADDING * 2)) * mm
    text_y = (bleed + (PADDING * 3)) * mm
    text = canvas.beginText(text_x, text_y)
    text.textLine("Template generated by KDPCoverGen")
    text.textLine("https://github.com/craig552uk/kdpcovergen")
    canvas.drawText(text)
